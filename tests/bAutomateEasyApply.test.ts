...
const keywords: string[] = JSON.parse(fs.readFileSync('./selectors/allEasyApplyKey.json', 'utf-8')).keywords;
...
async function handleEasyApplyPopup(page: Page) {
  ...
  // Blue Buttons
  const nextButton = await page.locator("//div[contains(@role,'dialog')]//following::button[contains(.,'Next')]");
  const reviewButton = await page.locator("//div[contains(@role,'dialog')]//following::button[contains(.,'Review')]");
  const submitApplication = await page.locator("//div[contains(@role,'dialog')]//descendant::button[contains(.,'Submit application')]");
  const doneButton = await page.locator("//div[contains(@role,'dialog')]//descendant::button[contains(.,'Done')]");
  ...
  const allYesRadioButtons = await page.locator("//div[contains(@role,'dialog')]//descendant::div[contains(@data-test-text-selectable-option,'0')]");

  // All questions
  const allQuestionsInput = await page.locator("/div[contains(@role,'dialog')]//descendant::input/preceding-sibling::label");
  const allQuestionsDropdown = await page.locator("/div[contains(@role,'dialog')]//descendant::select/preceding-sibling::label");
...
const handleEasyApplyPopup1 = async (page: Page) => {
  ...
  const allYesRadioButtons = page.locator('div[role="dialog"] input[type="radio"] + label:has-text("Yes"), div[role="dialog"] input[type="radio"] + label:has-text("True")');
